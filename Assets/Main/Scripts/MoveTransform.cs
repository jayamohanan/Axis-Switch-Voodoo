...
public class MoveTransform : MonoBehaviour
{
    ...
    private bool showedInstruction;
    ...
    {
        Tweak.Instance.SetToggle(1, showPath);
        Tweak.Instance.SetToggle(2, vibrate);
        Tweak.Instance.SetIF(1, vibrationAmount);
        set = true;
    }
    Vector3 rayOrigin;
    ...
    public void OnNextLevel()
    {
        ...
        else if (levelManager.currentLevel < 11)
        {
            rotateSpeed = 270;
            moveSpeed = 22.5f;
        }
        else if (levelManager.currentLevel < 16)
        {
            rotateSpeed = 290f;
            moveSpeed = 25f;
        }
        else if (levelManager.currentLevel < 30)
        {
            rotateSpeed = 310;
            moveSpeed = 27.5f;
        }
        ...
        giveBonus = false;
        axisList = gameManager.axisList;
        axisCount = axisList.Count;
        currentAxisIndex = 0;
        nextAxisIndex =1;
        currentAxis = axisList[currentAxisIndex];
        currentAxis.GetComponent<AxisScript>().counted = true;
        if (axisCount > nextAxisIndex)
        {
        ...
        rotationDirection = 1;
        //if (currentAxis.GetComponent<AxisScript>().biDirectional)
        //    rotationDirection = (Random.Range(0,2)==0?1:-1);
        if (currentAxis.GetComponent<AxisScript>().antiClockWise)
            rotationDirection = -1;

        targetPos = currentAxis.position;
        axisList[0].GetComponent<MeshRenderer>().material.color = axisTouchColor;
        bottomPosition = (targetPos + Vector3.forward * axisList[0].GetComponent<AxisScript>().rotateRadius).z;
        topPosition = gameManager.currentLevelTransform.Find("FinishPoint").position.z;
        distanceToTop = topPosition - bottomPosition;
        previousZ = bottomPosition;
        radius = currentAxis.GetComponent<AxisScript>().rotateRadius;
        cameraFollow.OnAxisChange();
        RotateAroundAxis(targetPos);
    }
    ...
    void Update()
    {
        if(!showedInstruction && cameraFollow.cameraAtStart)
        {
        ...
        {
            float distanceTravelled = (transform.position.z - bottomPosition) / distanceToTop;
            ScoreManager.Instance.SetDistanceSlider(distanceTravelled);
        }
        if(giveBonus)
        if(nextAxisIndex!=-1)
        if (move)
        {
                distance_CurrentAxis = Vector3.Distance(currentAxis.position, transform.position);
                if(distance_CurrentAxis > distance_NextAxis)
                {
                    nextAxis.GetComponent<AxisScript>().counted = true;
                    nextAxis.GetComponent<MeshRenderer>().material.color = axisTouchColor;
                    ShowScoreCollectionAnim(scoreCollectionTextPrefab, mainCam.WorldToScreenPoint(nextAxis.position), "+2",150, 1);
                        //GameObject skipScoreTextObject = Instantiate(scoreCollectionTextPrefab, gameManagerCanvas);
                        //skipScoreTextObject.transform.position = mainCam.WorldToScreenPoint(nextAxis.transform.position);
                        //LeanTween.alphaText(skipScoreTextObject.GetComponent<RectTransform>(), 0, 0.9f).setEase(LeanTweenType.easeOutCubic);
                        //LeanTween.moveY(skipScoreTextObject, skipScoreTextObject.transform.position.y+100, 1).setEase(LeanTweenType.easeOutCubic).setOnComplete(() =>
                        //{
                        //    Destroy(skipScoreTextObject);
                        //});
                        print("giveBonus true");
                    ShowScoreCollectionAnim(axisSkipTextPrefab, skipScoreStartPos, "AXIS SKIPPED +2", 150, 1.5f);

                    //GameObject axiskipTextObject = Instantiate(axisSkipTextPrefab, gameManagerCanvas);
                    //axiskipTextObject.transform.position = skipScoreStartPos;
                    //LeanTween.alphaText(axiskipTextObject.GetComponent<RectTransform>(), 0.1f, 1.4f).setEase(LeanTweenType.easeOutCubic);
                    //LeanTween.moveY(axiskipTextObject, skipScoreTargetHeight, 1.5f).setEase(LeanTweenType.easeOutCubic).setOnComplete(() =>
                    //{
                    //    Destroy(axiskipTextObject);
                    //});

                    ScoreManager.Instance.IncrementScore(2);
                    AudioManager.Instance.PlayTapSound();
                    if (axisCount> (nextAxisIndex + 1))
                    {
                        nextAxisIndex++;
                        nextAxis = axisList[nextAxisIndex];
                        distance_NextAxis = Vector3.Distance(currentAxis.position, nextAxis.position);
                    }
                    else
                    {
                        nextAxisIndex = -1;
            ...
            if (other.tag == "Axis")
            {
                ...
                for(int i=lastAxisIndex+1; i< currentAxisIndex; i++)
                {
                    ...
                    if (!axisScript.counted)
                    {
                        ...
                        ShowScoreCollectionAnim(axisSkipTextPrefab, skipScoreStartPos, "AXIS SKIPPED +2", 150, 2f);
